...
typedef struct {
	int conv_cnt;                 //количество операций свертки, которые будут выполнены

	int thread_cnt;               //общее количество потоков, используемых при выполнении операций свертки
	int thread_number;            //номер текущего потока, используемого при выполнении операций свертки

	int new_im_size;              //размер нового изображения или матрицы, на которую будет производиться операция свертки
	int res_size;                 //размер результата операции свертки

	double *new_img_matrix;       //новое изображение или матрица, на которую будет производиться операция свертки
	double *res_matrix;           //результат операции свертки
...
void print(int size, double *matrix) {                                //вывод матрицы
...
void print2(int size, double *matrix) {                               //вывод матрицы в файл
	FILE *f;
...
void *convolution(void *a) {
	...
	int n_rest = n_elems % args->thread_cnt; //остаток операций, который необходимо разделить между потоками

	int start, end;                             //диапазон операций, которые будет выполнять текущий поток
	if (args->thread_number == 0) {
		start = args->thread_number * n_opers; //Начальный индекс операций для первого потока
		end = (args->thread_number + 1) * n_opers + n_rest; //Конечный индекс операций для первого потока (с учетом остатка)
	} else {
		start = args->thread_number * n_opers + n_rest;   //для остальных потоков
		end = (args->thread_number + 1) * n_opers + n_rest;
	}

	int row_offset = 0, col_offset = 0;   //начальной позиции матрицы изображения
	while (CONV_SIZE + row_offset <= args->new_im_size) {   //перебор всех возможных матриц изображения
		col_offset = 0;   //сброс смещения столбцов
		while (CONV_SIZE + col_offset <= args->new_im_size) {
			for (int op = start; op < end; ++op) {   // Цикл по операциям свертки, выполняемым текущим потоком
				int row = op / CONV_SIZE;     //номер строки для текущей операции
				int col = op % CONV_SIZE;

				args->res_matrix[get_ind(row_offset, col_offset, args->res_size)] += conv_matrix[row][col] * 
				args->new_img_matrix[get_ind(row_offset + row, col_offset + col, args->new_im_size)];   // Выполнение операции свертки
			}
		++col_offset;
		}
	++row_offset;
...
int main(int argc, char const *argv[])
{
	...
	memcpy(new_img_matrix, img_matrix, sizeof(double) * new_im_size * new_im_size);     //копирование
	...
	for (int i = 0; i < conv_cnt; ++i) { //применить k раз свертку
		...
		}  //В каждой итерации обновление размера нового изображения, присваивание новой матрицы новому изображению, пересчет размера результата свертки и выделение памяти под него
