...
int main (void)
{
    ...
    hashmap_init(&answer_count_map, hashmap_hash_string, strcmp); //для подсчета ответов
    hashmap_init(&nodes_ping_map, hashmap_hash_string, strcmp); //для отслеживания пингов
    ...
    mqn_init(&CONTROL_NODE); //Инициализирует и привязывает структуру CONTROL_NODE через библиотеку для сетевого обмена сообщениями
    mqn_bind(&CONTROL_NODE, main_address, ping_address);

    pthread_t receive_thread; //обработка получения сообщений
    pthread_t heartbeat_thread; //отправка пингов
...
void gen_address(char *rep_address, char *req_address) { //генерирует адреса для сервера и клиента в сокетном взаимодействии по протоколу TCP
    char *buff = int_to_str(address_shift++);
    rep_address[0] = '\0';
    strcat(rep_address, "tcp://*:");
    strcat(rep_address, buff); //генерируется адрес для сервера в формате "tcp://*:port", где порт - это значение, преобразованное из числа address_shift
    if (req_address != NULL) { //предоставлен адрес для клиента
        req_address[0] = '\0';
        strcat(req_address, "tcp://localhost:");
        strcat(req_address, buff); //генерируется адрес для клиента в формате "tcp://localhost:port"
    }
    free(buff);
}

char* gen_uuid() {  //генерирует уникальный идентификатор (UUID) и возвращает его в виде строки
...
void cmd_create(int id) { //добавление узла в бинарное дерево
    ...
    if (pid == 0) { //в дочернем выполняем compute_node
        ...
        exit(errno);
    } else { //в родительском добавляем новый узел в бинарное дерево
...
void cmd_remove(int id) { //удаление узла из бинарного дерева
    b_tree_node removed_node = btree_find(&TREE, id); //находим узел
...
bool cmd_heartbeat(int time) { //проверка времени
    if (time < 0) {
...
void rec_create(int id, int code, char* answer, int count_answers) { //различные сценарии создания узла в дереве
...
void rec_remove(char* command, int id, int code, char* answer, int count_answers) { //удаляет элементы из списка removelist
...
void rec_exec(int id, int code, char* answer, int count_answers) { //обработка результата выполнения команды
    if (code == CODE_ERROR_NOT_FOUND && count_answers == 0 || code == CODE_ERROR_CUSTOM)
        printf("Error:%d: %s\n", id, answer);
    else if (code == CODE_OK)
        printf("Ok:%d: %s\n", id, answer);
